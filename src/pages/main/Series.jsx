...
export default function Series() {
  ...
  const availableGens = useMemo(() => {
    const set = new Set()
    series.forEach((t) => t.genres.forEach((g) => set.add(g)))
    return ['Todos', ...Array.from(set).sort()]
  }, [series])

  const filtered = useMemo(() => {
    let out = genre === 'Todos' ? series : series.filter((t) => t.genres.includes(genre))
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      <header className="mb-8 animate-slide-up">
        <h1 className="text-3xl md:text-5xl font-display font-extrabold tracking-tight">Séries</h1>
        <p className="mt-2 text-dark-500 dark:text-dark-400">Maratonas incríveis esperando por você</p>
        <p className="mt-2 text-sm text-dark-500 dark:text-dark-400">{series.length} séries no catálogo</p>
      </header>
      ...
      {filtered.length === 0 ? (
        <div className="py-20 text-center">
          <Icon name="tv" size={40} className="mx-auto text-dark-300 dark:text-dark-600" />
          <p className="mt-4 text-lg font-semibold">Nenhuma série encontrada</p>
